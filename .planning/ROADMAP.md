...
**Plans**: 5 plans
Plans:
- [x] 03-01-PLAN.md — Backend foundation: entities + DTOs + IStockMovementRepository (JOIN'd, zero N+1) + CreateMovementRequestValidator + 5 typed exceptions + ExceptionHandlingMiddleware doc (MOVE-02/04/05/07/08/09/10/11)
- [x] 03-02-PLAN.md — Frontend feature primitives: types + api (FRONT-12 Idempotency-Key) + Zod schemas + useStockMovements composable + extend labels.ts with movementTypeLabel (FRONT-12)
- [x] 03-03-PLAN.md — Backend service + controller + DI: transactional CreateAsync (SELECT FOR UPDATE + idempotency replay + 23505 race-window) + StockMovementsController with 3 operationIds + Program.cs (MOVE-01..11)
- [x] 03-04-PLAN.md — Frontend components: InboundForm + OutboundForm (UX-08 Disponível + balance pre-check) + ConfirmOutboundModal (CONF-01 verbatim) + MovementHistory (4-state + filters) + StockMovementsPage rewrite (tab strip ARIA) (CONF-01)
- [x] 03-05-PLAN.md — End-to-end smoke: docker compose stack + MOVE-01..11 curl matrix + zero-N+1 pg_stat_statements evidence + Swagger contract + SPA + CONF-01 grep + README touchup
**UI hint**: yes
